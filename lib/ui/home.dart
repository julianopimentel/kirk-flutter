...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadSavedValues(BuildContext context) async {
    ...
    if (permissionsJson != null) {
      ...
      permissions =
          Permission.permissionsFromJson(json.decode(permissionsJson));
    }
    else {
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    buildHeaderText(nameConta, context, showMultiAccounts),
                  ],
                ),
                Spacer(),
                Row(
  ...
  double calculateMaxTextLength(BuildContext context, bool showMultiAccounts) {
    ...
    double iconSpace = showMultiAccounts ? 40.0 : 1.0; // ajuste conforme necessário
    ...
    TextStyle textStyle = TextStyle(fontSize: 12); // ajuste conforme necessário
  ...
  List<MenuItem> buildMenuItems(List<Permission> permissions) {
    ...
    permission.features == 'PERSON' && permission.ler == true)) {
      menuItems.add(MenuItem(
          Icons.group_outlined, 'Grupo Familiar', route: 'personPage'));
    }
    if (permissions.any((permission) =>
    ...
    permission.features == 'FINANCIAL' && permission.ler == true)) {
      menuItems.add(MenuItem(
          Icons.qr_code_outlined, 'Enviar Oferta', route: 'EnviarOfertaPage'));
    }

    if (permissions.any((permission) =>
    permission.features == 'PRAYER' && permission.ler == true)) {
      menuItems.add(MenuItem(
          Icons.support_agent_outlined, 'Pedir Oração', route: 'prayerPage'));
    }
    ...
    menuItems.add(
        MenuItem(Icons.location_history, 'Meus dados', route: 'meusDadosPage'));
    menuItems.add(MenuItem(
        Icons.add_location_outlined, 'Endereço', route: 'enderecoPage'));
    // Continue adicionando itens com base nas permissões
    return menuItems;
  }

  Widget buildHeaderText(String nameConta, BuildContext context, bool showMultiAccounts) {
